...
[Clash.Meta](https://github.com/MetaCubeX/Clash.Meta) Windows Tun

[Box4Magisk](https://github.com/CHIZI-0618/box4magisk) Android Tproxy

[Clash.Meta Docs](https://wiki.metacubex.one/config/)
...
- Clash.Meta.StartUp.vbs - silent start VBS
- Clash.Meta.StartUp.xml - Windows Task Scheduler backup file
...
3. change Clash.Meta core file path in `Clash.Meta.StartUp.vbs`
4. change dir paths in `update-geo-files.bat` 、 `update-metacubexd.bat`.
5. modify `clash.meta-windows-amd64.exe`'s compatiable settings, tick "admin permission"。
6. db-click `Clash.Meta.StartUp.vbs` to run, allow admin permission.
...
1. Task Scheduler.
2. import `Clash.Meta.StartUp.xml` or new task to run `Clash.Meta.StartUp.vbs`.
3. change name, file path, triger, condition and so on if needed.
4. in "General" tab, tick 'admin/higherst permission', save, done.

**End Task:**

in task manager, terminate `clash.meta-windows-amd64.exe`.
